...
depth_plot <- ggplot(
  ...
  scale_x_log10(
    breaks = c(0.0001, 0.001, 0.005, 0.01, 0.05, 0.1),
  ...
  theme(
    ...
    # Remove panel border, but keep axis
...
ggsave("../output/depth_plot.png", depth_plot, width = 8, height = 4)
...
select_mew_read_threshold_data_with_cost <- select_mew_read_threshold_data %>%
  ...
  mutate(
    total_cost_per_year = calc_total_cost_per_year(
